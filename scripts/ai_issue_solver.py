...
def main():
    ...
    prompt = f"Create a PR to solve this GitHub Issue. Title: {ctx['title']}. Body: {ctx['body']}. Respond ONLY with JSON: [{{'path': 'file', 'content': 'content'}}]"
    
    # 2. Call AI (Resilient GET/POST)
    result = ""
    
    # Strategy 1: Pollinations GET (Ultra-Reliable for small prompts)
    try:
    ...
    if not result or "[" not in result:
        comment(issue_no, f"AI Resolution failed. Result: {result[:200]}")
        return
    ...
    try:
        ...
        subprocess.run(["gh", "issue", "close", str(issue_no), "--comment", "🤖 AI fix proposed. PR created."])
