...
const memoryVisualizer = {
  ...
  showHistory: function (ticks = 10) {
    ...
    snapshots.forEach((snap) => {
      console.log(
                `[T:${snap.time}] Size: ${((snap.size || 0) / 1024).toFixed(2)} KB, Energy: ${snap.energy}`
      )
    })
  ...
  showMap: function () {
    ...
    for (const roomName in Memory.map.rooms) {
      if (
        ...
        console.log(
                    ` - ${roomName}: Owner=${owner}, Level=${info.controller?.level ?? 0}, Sources=${info.sources}`
        )
      }
    }
  },
