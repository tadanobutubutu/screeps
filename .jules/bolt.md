...
**Learning:** Replacing expensive engine-level collections like `room.find(FIND_SOURCES)` with pre-warmed cache equivalents (`cache.getSources(room)`) inside high-frequency planning functions (`utils.planning.js:findBestSpawnPosition`, `utils.planning.js:planRoadNetwork`) yields significant CPU performance gains by circumventing array allocation overheads and engine-level searches. However, dependencies on caches require unit test mocks to properly specify `room.find()` defaults when `getSources` falls back to its default behavior, otherwise `room.find is not a function` runtime errors will surface during tests.
