...
const adaptiveSystem = {
  ...
  evaluate: function() {
    ...
    if (newMode !== Memory.adaptive.currentMode) {
      ...
      Memory.adaptive.modeHistory.push({
        ...
        to: newMode,
        reason: this.getModeChangeReason(newMode, cpuUsagePercent, cpuBucket, memoryUsagePercent)
      });
  ...
  getModeName: function(mode) {
    switch(mode) {
      case this.MODE.EMERGENCY: return 'emergency';
      case this.MODE.MINIMAL: return 'minimal';
      case this.MODE.NORMAL: return 'normal';
      case this.MODE.FULL: return 'full';
      default: return null;
    }
  },
  ...
  showDashboard: function() {
    ...
    console.log('');
    console.log('CPU Used: ' + cpuUsed.toFixed(2) + '/' + cpuLimit + ' (' + ((cpuUsed/cpuLimit)*100).toFixed(1) + '%)');
    ...
    const allFeatures = [
      'basicRoles', 'spawn', 'defense', 'logging',
      'gamification', 'emotions', 'memoryVisualizer',
      'visualEffects', 'autoEvolution', 'tutorial',
      'socialInteractions', 'advancedRoles'
    ];
    ...
    const total = stats.emergencyCount + stats.minimalCount + stats.normalCount + stats.fullCount;
    if (total > 0) {
    ...
    if (Memory.adaptive.modeHistory.length > 0) {
      ...
      for (let i = 0; i < recentHistory.length; i++) {
        ...
        console.log('  [' + h.time + '] ' + this.getModeName(h.from) + ' → ' + 
                    this.getModeName(h.to) + ' (' + h.reason + ')');
      }
    }
  },
