...
def main():
    ...
    res = subprocess.run(
        ["git", "for-each-ref", "--format=" + fmt, "refs/remotes/origin"],
        capture_output=True,
        text=True,
    )
    ...
    for line in branches_raw:
        ...
        if age_days < 14:
            continue  # 最近のものは絶対に消さない

        # --- コンテキスト情報の収集 ---

        # main との差分 (Ahead/Behind)
        dist_res = subprocess.run(
            [
                "git",
                "rev-list",
                "--left-right",
                "--count",
                f"origin/main...{full_name}",
            ],
            capture_output=True,
            text=True,
        )
        ...
        diff_res = subprocess.run(
            ["git", "diff", "--name-only", "origin/main", full_name],
            capture_output=True,
            text=True,
        )
        files_changed = (
            diff_res.stdout.strip().split("\n") if diff_res.returncode == 0 else []
        )
        ...
        # ボットかどうか
        is_bot = "github.com" in author_email.lower() or author_name.lower() in [
            "github actions",
            "github-actions",
        ]
    ...
    for branch in to_delete:
        if branch.lower() in ["main", "master", "prod", "gh-pages", "ptr"]:
            continue
