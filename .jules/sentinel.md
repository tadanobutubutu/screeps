...
**Prevention:** Instead, use a cryptographically secure random number generator like Node's `crypto.randomBytes()` with a fallback mechanism if `crypto` is unavailable in the environment (e.g., Screeps sandbox).  
