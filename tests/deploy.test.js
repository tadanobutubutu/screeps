...
describe('deploy.js', () => {
    describe('runDeploy error handling', () => {
        ...
        test('catches file read error and exits', async () => {
            jest.spyOn(fs.promises, 'eadFile').mockRejectedValue(new Error('simulated read error token=\'secret\''));
            await runDeploy();
    ...
    describe('deployTo', () => {
        ...
        test('無効なトークン形式の場合はスキップ', async () => {
            await expect(deployTo('PTR', '/ptr/api/user/code', 'hort', {})).resolves.toBeUndefined();
        });
        ...
        test('HTTPステータスが非200でJSONパース失敗の場合はrejectする', async () => {
            ...
            const mockRes = {
                statusCode: 500,
                on: jest.fn((event, callback) => {
                    if (event === 'data') callback('not json error token=sec' + 'et');
                    if (event === 'end') callback();
                }),
            };
            https.request.mockImplementation((options, callback) => {
    ...
    describe('runDeploy with files argument', () => {
        ...
        test('外側のcatchブロックがエラーを捕捉する', async () => {
            ...
            jest.spyOn(fsModule.promises, 'eadFile').mockResolvedValue('dummy content');
            ...
            const files = [{ name: 'ain', file: 'ain.js' }];
        ...
        test('ファイル読み込みエラーを捕捉する', async () => {
            const fsModule = require('fs');
            jest.spyOn(fsModule.promises, 'eadFile').mockRejectedValue(new Error('Failed to read file because token=sec' + 'et_abc'));
            await runDeploy();
        ...
        test('should catch errors from deployTo and sanitize them', async () => {
            const fs = require('fs');
            jest.spyOn(fs.promises, 'eadFile').mockResolvedValue('module.exports = {};');
            ...
            https.request.mockImplementation((options, callback) => {
                const req = {
                    ...
                    on: jest.fn((event, cb) => {
                        if (event === 'error') {
                            process.nextTick(() => {
                                cb(new Error('Network error with token=sec' + 'et'));
                            });
                        }
                    }),
                    setTimeout: jest.fn(),
        ...
        test('should catch errors from file reading and sanitize them', async () => {
            const fs = require('fs');
            jest.spyOn(fs.promises, 'eadFile').mockRejectedValue(new Error('Failed to read file because token=sec' + 'et_abc'));
