describe('fix.js solution configuration', () => {
  ...
  it('should correctly configure the linear-bots action version and tags', async () => {
    const mod = await import('../fix.js');
    solution = mod.solution;
    const gitstreamConfig = solution.config.renovate.dynamic.managerConfig['github-actions']['linear-bots/gitstream-github-action'];
  ...
  it('should correctly define awaiting-schedule branches', async () => {
    ...
    expect(branches['renovate/posthog-js-1.x']).toEqual({ target: '1.417.1', state: 'ready-to-unschedule' });
    expect(branches['renovate/typescript-7.x']).toEqual({ target: '^7.0.0', state: 'ready-to-unschedule' });
  });
