...
describe('src/roles/miner', () => {
  ...
  describe('_findSourceContainer', () => {
    test('コンテナのキャッシュがnullの場合はnullを返す', () => {
      ...
      const creep = {
        name: 'miner_null_cache',
        memory: { sourceId: 'src_null_cache' },
        room: roomMock,
        pos: new RoomPosition(5, 5, 'W0N0'),
        harvest: jest.fn().mockReturnValue(global.OK),
        drop: jest.fn(),
    ...
    test('コンテナのキャッシュがあるがオブジェクトが存在しない場合、キャッシュを無効化して再スキャンする', () => {
      ...
      const creep = {
        name: 'miner_stale_cache',
        memory: { sourceId: 'src_stale_cache' },
        room: roomMock,
        pos: new RoomPosition(5, 5, 'W0N0'),
        harvest: jest.fn().mockReturnValue(global.OK),
        drop: jest.fn(),
    ...
    test('コンテナのキャッシュがありオブジェクトが存在する場合、キャッシュからコンテナを返す', () => {
      ...
      const creep = {
        name: 'miner_hit_cache',
        memory: { sourceId: 'src_hit_cache' },
        room: roomMock,
        pos: new RoomPosition(5, 5, 'W0N0'),
        harvest: jest.fn().mockReturnValue(global.OK),
        drop: jest.fn(),
