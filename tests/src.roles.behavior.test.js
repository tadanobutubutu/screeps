...
describe('src roles behaviors', () => {
  ...
  describe('builder', () => {
    test('建設ターゲットを選択してbuildする', () => {
      const siteA = { id: 'ext', structureType: STRUCTURE_EXTENSION, pos: { x: 10, y: 10, getRangeTo: jest.fn().mockReturnValue(2) }, progress: 10, progressTotal: 20 };
      const siteB = { id: 'road', structureType: STRUCTURE_ROAD, pos: { x: 11, y: 11, getRangeTo: jest.fn().mockReturnValue(3) }, progress: 5, progressTotal: 10 };
      ...
      const creep = {
        ...
        repair: jest.fn(),
        room: { name: 'W0N0', visual: { text: jest.fn() }, find: jest.fn().mockReturnValue([]), controller: { pos: { x: 1, y: 1 } } },
  ...
  describe('defender', () => {
    test('敵がいるとき遠距離攻撃を実行する', () => {
      const enemy = { hits: 50, hitsMax: 100, getActiveBodyparts: jest.fn().mockReturnValue(0), pos: { x: 3, y: 3 } };
    ...
    test('敵数が多いとセーフモード判定がtrueになる', () => {
      ...
      const hostile = { hits: 100, hitsMax: 100, getActiveBodyparts: jest.fn().mockReturnValue(1) };
    ...
    test('コンテナがある場合は移動して採掘する', () => {
      const room = {
        ...
        find: jest.fn().mockReturnValue([
          { structureType: STRUCTURE_CONTAINER, pos: { x: 11, y: 10 }, hits: 1000, hitsMax: 2000 },
        ]),
  ...
  describe('upgrader', () => {
    test('ストレージからエネルギーを取得する', () => {
      const storage = { store: { [RESOURCE_ENERGY]: 2000 }, withdraw: jest.fn().mockReturnValue(OK) };
