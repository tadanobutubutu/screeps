...
class EmotionSystem {
    ...
    static _getEnergyEmotion(creep) {
        ...
        const energyPercent = store.getUsedCapacity(RESOURCE_ENERGY) / store.getCapacity(RESOURCE_ENERGY);
    ...
    static _getStuckEmotion(creep) {
        ...
        if (lastPos && pos.x === lastPos.x && pos.y === lastPos.y) {
            ...
            if (creep.memory.stuckCounter > 3) return RESULT_STUCK;
        } else {
    ...
    static celebrate(creep, achievement) {
        ...
        creep.memory.emotions.achievements.push({ name: sanitizedName, tick: Game.time });
