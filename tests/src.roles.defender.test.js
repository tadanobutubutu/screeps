...
describe('src/roles/defender', () => {
  ...
  test('敵がいるとき攻撃し回復する', () => {
    ...
    const creep = {
      ...
      getActiveBodyparts: jest.fn()
        .mockImplementation((part) => (part === global.RANGED_ATTACK || part === global.ATTACK || part === global.HEAL ? 1 : 0)),
  ...
  test('セーフモード判定が敵数と防衛数でtrueになる', () => {
    ...
    global.Game.creeps = {
      d1: { room, memory: { role: 'harvester' }, getActiveBodyparts: jest.fn().mockReturnValue(0) },
    };
  ...
  test('getBodyは遠距離型の構成を返す', () => {
    ...
    expect(defender.getBody(200, false)).toEqual([global.TOUGH, global.ATTACK, global.MOVE, global.MOVE]);
  });
});
