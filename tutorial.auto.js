...
const autoTutorial = {
    ...
    step2_harvestEnergy: function () {
        ...
        for (const roomName in Game.rooms) {
            sourcesCache[roomName] = Game.rooms[roomName] && Game.rooms[roomName].find ? Game.rooms[roomName].find(FIND_SOURCES) : [];
        }
    ...
    autoStep: function () {
        ...
        for (const roomName in Game.rooms) {
            ...
            sourcesCache[roomName] = room && room.find ? room.find(FIND_SOURCES) : [];
            sitesCache[roomName] = room && room.find ? room.find(FIND_CONSTRUCTION_SITES) : [];
            hostilesCache[roomName] = room && room.find ? room.find(FIND_HOSTILE_CREEPS) : [];
        }
