...
def main():
    ...
    pr_body = (
        f"Closes #{issue_no}\n\n"
        "## Multi-Agent Fix\n\n"
        "This PR was generated by querying **all available free LLM APIs simultaneously** "
        "(Pollinations, Kilo Gateway, OVH Anonymous, Puter.js, HuggingFace, AI Horde, Gemini, OpenRouter).\n\n"
        f"{test_note}\n\n"
        "**Security Note:** For safety, the AI issue solver no longer executes untrusted code locally. It only performs syntax validation.\n"
        "Please review the PR and let GitHub Actions run the full test suite in a sandboxed environment.\n\n"
