...
## 2026-08-07 - Cross-Tick Target Caching and Array Loop Refactoring
**Learning:** In Screeps, array methods like `.filter` allocate closures and put pressure on the garbage collector in hot paths like creep tick processing. Replacing them with indexed `for` loops saves CPU. Furthermore, when caching the results of expensive operations like `findClosestByRange` across ticks using `creep.memory`, it is essential to ensure the cached entity remains valid (e.g., checking if it still has `hits` and is in the same room) before skipping the search, lest the creep get stuck pathing to an invalid or out-of-bounds target.
**Action:** When finding hostile targets, use indexed `for` loops for filtering valid targets. Always add a validation step (`!target.hits || target.room.name !== creep.room.name`) before reusing cached targets to ensure stability across ticks.
