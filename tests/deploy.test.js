...
describe('deploy.js', () => {
    describe('validateToken', () => {
        test('有効な32文字の16進数トークンを許可', () => {
            const result = validateToken('a1b2c3d4e5f60789a1b2c3d4e5f60789');
            expect(result).toBe(true);
        });

        test('31文字の短いトークンを無効', () => {
            const result = validateToken('a1b2c3d4e5f60789a1b2c3d4e5f6078');
            expect(result).toBe(false);
        });

        test('33文字の長いトークンを無効', () => {
            const result = validateToken('a1b2c3d4e5f60789a1b2c3d4e5f607890');
            expect(result).toBe(false);
        });

        test('16進数以外の文字を含むトークンを無効', () => {
            const result = validateToken('z1b2c3d4e5f60789a1b2c3d4e5f60789');
            expect(result).toBe(false);
        });
    ...
    describe('deployTo', () => {
        ...
        test('デプロイ成功時にresolveする', async () => {
            ...
            const validToken = 'a1b2c3d4e5f60789a1b2c3d4e5f60789';
            await expect(
