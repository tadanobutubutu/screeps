  ...
  describe('commitAndPush', () => {
    it('should run git commands', () => {
      ...
      expect(require('child_process').execFileSync).toHaveBeenCalledWith(
        'git',
        ['add', '.all-contributorsrc', 'README.md'],
        { stdio: 'inherit' }
      )
      expect(require('child_process').execFileSync).toHaveBeenCalledWith(
        'git',
        ['commit', '-m', 'docs: add testuser as a contributor'],
        { stdio: 'inherit' }
      )
      expect(require('child_process').execFileSync).toHaveBeenCalledWith(
        'git',
    ...
    it('should log warning on failure', () => {
      require('child_process').execFileSync.mockImplementation(() => {
    ...
    it('should reject invalid username for commit', () => {
      ...
      expect(console.warn).toHaveBeenCalledWith(
        '⚠️  Invalid username for commit: ../malicious'
      )
  ...
  describe('main', () => {
    ...
    it('should skip if no .all-contributorsrc', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        'ℹ️  Skipping contributor addition (no .all-contributorsrc)'
      )
    })
    ...
    it('should skip if user is already a contributor', async () => {
      ...
      require('fs').readFileSync.mockReturnValue(
        JSON.stringify({ contributors: [{ login: 'testuser' }] })
      )
    ...
    it('should add contributor, update config, and commit', async () => {
      ...
      const mockResponse = {
        ...
        on: jest.fn((event, cb) => {
          if (event === 'data') {
            cb(JSON.stringify({ login: 'testuser', name: 'Test User' }))
          }
          if (event === 'end') cb()
        })
      }
