...
def main():
    ...
    t_gemini = (
        threading.Thread(
            target=call_gemini_api, args=(prompt, gemini_api_key, results, 0)
        )
        if gemini_api_key
        else None
    )
    t_openrouter = (
        threading.Thread(
            target=call_openrouter, args=(prompt, openrouter_token, results, 1)
        )
        if openrouter_token
        else None
    )
    t_pollinations = threading.Thread(
        target=call_pollinations_ai, args=(prompt, results, 2)
    )
    t_huggingface = threading.Thread(
        target=call_huggingface_anonymous, args=(prompt, results, 3)
    )
    t_aihorde = threading.Thread(target=call_aihorde, args=(prompt, results, 4))

    active_threads = [
        t
        for t in [t_gemini, t_openrouter, t_pollinations, t_huggingface, t_aihorde]
        if t is not None
    ]
