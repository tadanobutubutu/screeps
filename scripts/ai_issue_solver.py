...
def main():
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        if openrouter_token:
            ...
            threads.append(
                threading.Thread(
                    target=run_provider,
                    args=(1, call_openrouter, prompt, openrouter_token),
                )
            )
            ...
            threads.append(
                threading.Thread(
                    target=run_provider,
                    args=(2, call_openrouter, prompt, openrouter_token),
                )
            )

        threads.append(
            threading.Thread(
                target=run_provider, args=(3, call_pollinations_post, prompt, "openai")
            )
        )
        threads.append(
            threading.Thread(
                target=run_provider,
                args=(4, call_pollinations_get, prompt, "openai-fast"),
            )
        )
        threads.append(
            threading.Thread(target=run_provider, args=(5, call_kilo_gateway, prompt))
        )
        threads.append(
            threading.Thread(target=run_provider, args=(6, call_ovh_anonymous, prompt))
        )
        threads.append(
            threading.Thread(
                target=run_provider, args=(7, run_provider, 7, ask_aihorde, prompt)
            )
        )  # AI Horde in parallel thread
        ...
        for i, candidate in enumerate(results):
            ...
            if test_run.returncode == 0:
                ...
                failed_candidates_feedback.append(
    ...
    subprocess.run(
        [
            ...
            "--no-verify",
            "-m",
            f"fix: resolve issue #{issue_no} using multi-agent concurrent consensus",
        ]
    )
    ...
    subprocess.run(
        [
            ...
            "create",
            "--title",
            f"Fix #{issue_no} - Concurrent Multi-Agent",
            "--body",
            f"Closes #{issue_no}\n\nThis fix was resolved using concurrent execution of multiple models, validated by local unit tests.",
            "--head",
            branch,
            "--base",
            "main",
