...
describe('system.adaptive', () => {
    ...
    test('logModeChangeが正しくログを出力する', () => {
        ...
        adaptiveSystem.logModeChange(0, 3, stats);

        expect(consoleSpy).toHaveBeenCalledWith('\n🔄 === ADAPTIVE SYSTEM MODE CHANGE === 🔄');
        expect(consoleSpy).toHaveBeenCalledWith('From: EMERGENCY → To: FULL');
        expect(consoleSpy).toHaveBeenCalledWith('CPU Usage: 85.5%');
        expect(consoleSpy).toHaveBeenCalledWith('CPU Bucket: 5000/10000');
        expect(consoleSpy).toHaveBeenCalledWith('Memory Usage: 60.2%');
