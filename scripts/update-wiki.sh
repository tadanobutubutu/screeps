...
# Create wiki-repo directory if checkout failed
if [ ! -d "wiki-repo" ]; then
  echo "📝 Wiki repo not found, initializing..."
  mkdir -p wiki-repo
  cd wiki-repo
  git init
  git remote add origin "https://github.com/${GITHUB_REPOSITORY}.wiki.git" || true
  cd ..
fi
...
[← Back to Home](Home) | [Next: Bot System →](Bot-System)
EOF
...
[← Back to Home](Home) | [Next: Error Handling →](Error-Handling)
EOF
...
[← Back to Home](Home) | [Next: API Reference →](API-Reference)
EOF
...
EOF
  ...
  echo "✅ Created default wiki pages"
fi
