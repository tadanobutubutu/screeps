...
- Randomness in Math.random() is vulnerable to PRNG prediction. Consider using `crypto.randomBytes()` wrapped in a `secureRandom()` helper for security-sensitive logic (or even visual effects if required to prevent pattern prediction). Retain `Math.random()` as a fallback for sandbox environments where `crypto` might be unavailable.
