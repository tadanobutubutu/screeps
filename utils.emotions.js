...
class EmotionSystem {
    ...
    static interact(creep1, creep2) {
        ...
        if (creep1.pos.inRangeTo(creep2, 1)) {
            ...
            // Security: Ensure mood is a finite number before arithmetic to prevent NaN propagation.
            const mood1 = Number.isFinite(creep1.memory.emotions.mood)
                ? creep1.memory.emotions.mood
                : MOOD_LEVELS.NEUTRAL;
            const mood2 = Number.isFinite(creep2.memory.emotions.mood)
                ? creep2.memory.emotions.mood
                : MOOD_LEVELS.NEUTRAL;
    ...
    static checkCreep(creepName) {
        ...
        console.log('Achievements:', emotions.achievements ? emotions.achievements.length : 0);
