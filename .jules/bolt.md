...
## 2025-02-19 - Robust Lazy Caching of Configuration Entries

**Learning:** In high-frequency loops, iterating over object properties using `Object.entries()` creates significant CPU overhead due to per-tick array allocation and iterator creation. While hoisting works for static constants, it can lead to brittle logic if the configuration object is swapped. Using a `WeakMap` to lazy-cache these results provides a robust, generic way to optimize iteration without sacrificing flexibility or risking memory leaks.
**Action:** Implement lazy-caching via `WeakMap` for `Object.entries()` or `Object.keys()` in hot paths where the target object is dynamic but typically stable across ticks.
