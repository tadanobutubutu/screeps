...
def get_repo_stats():
    ...
    commits = subprocess.run(["git", "log", "-n", "20", "--pretty=format:%h|%an|%as|%s"], capture_output=True, text=True).stdout
    return {
        "workflows": workflows,
        "roles": roles,
...
def main():
    ...
    if changelog_content:
        with open("CHANGELOG.md", "w") as f:
            f.write("# 📈 開発・進化の記録 (Strategic Evolution Log)\n\n" + changelog_content.strip())
    ...
    if perf_content:
        with open("PERFORMANCE_REPORT.md", "w") as f:
            f.write("# ⚡ AI 自律解析レポート (Intelligence Report)\n\n" + perf_content.strip())
