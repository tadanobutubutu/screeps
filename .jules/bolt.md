...
## 2024-06-02 - ⚡ Bolt Learning: `Array.prototype.filter` inside loops  
**Title:** Optimizing  
...
**Action:** Refactor selection logic to use a single-pass `for` loop and local variable to cache the `bestScore`. This ensures exactly $N$ scoring calls and avoids the minor overhead of the `reduce` callback.  
...
**Learning:** Multiple towers within the same room repeatedly call targeting logic (like `_selectRepairTarget`). Since game state is completely static during a single tick (actions resolve at the end), these redundant $O(N)$ searches are wasteful.  
...
**Learning:** Using `localeCompare` and inside-loop `.toLowerCase()` conversions for string sorting causes significant CPU and garbage collection overhead in Node.js because `localeCompare` is heavily internationalized and `.toLowerCase()` allocates $O(N \log N)$ temporary strings.  
**Action:** Replace `localeCompare` with direct string comparison (`<` and `>`), and pre-calculate lowercase keys using a Schwartzian transform (map-sort-map) to reduce string allocation overhead to $O(N)$.  
