...
const ADAPTIVE_DEFAULTS = {
    currentMode: SystemLevels.NORMAL,
    lastCheck: 0,
    modeHistory: [],
    stats: {
        emergencyCount: 0,
...
const VALID_MODES = new Set([SystemLevels.EMERGENCY, SystemLevels.MINIMAL, SystemLevels.NORMAL, SystemLevels.FULL]);
...
const FEATURE_CONFIG = {
    [SystemLevels.EMERGENCY]: {
        basicRoles: true,
        spawn: true,
        memoryCleanup: true,
    },
    [SystemLevels.MINIMAL]: {
        basicRoles: true,
        spawn: true,
        memoryCleanup: true,
        defense: true,
...
const adaptiveSystem = {
    ...
    evaluate: function () {
        ...
        if (Memory.adaptive && Game.time - (Memory.adaptive.lastCheck || 0) < 10) {
            ...
            return VALID_MODES.has(currentMode) ? currentMode : SystemLevels.NORMAL;
        }
        ...
        if (!VALID_MODES.has(Memory.adaptive.currentMode)) {
            Memory.adaptive.currentMode = SystemLevels.NORMAL;
        }
    ...
    logModeChange: function (oldMode, newMode, stats) {
        ...
        // Security: Use logger for consistent sanitization while satisfying test output expectations
    ...
    isEnabled: function (feature) {
        ...
        if (_configTick !== Game.time || !_currentConfig) {
            const mode = Memory.adaptive ? Memory.adaptive.currentMode : SystemLevels.NORMAL;
    ...
    _printResourceUsage: function (modeName) {
        ...
        console.log('Bucket: ' + cpuBucket + ' (' + (cpuBucket / 100).toFixed(1) + '%)');
        console.log(
            'Memory: ' +
    ...
    _printRecentHistory: function () {
        if (Memory.adaptive.modeHistory.length > 0) {
            ...
            for (let i = 0; i < recentHistory.length; i++) {
                ...
                console.log(
                    ' - ' + h.time + ': ' + fromName.toUpperCase() + ' → ' + toName.toUpperCase() + ' (' + reason + ')'
                );
            }
        }
    },
    ...
    setMode: function (mode) {
        ...
        if (
            ...
            console.log('Invalid mode. Please use 0-3.');
            return;
        }
        ...
        console.log('System mode forced to: ' + this.getModeName(numericMode).toUpperCase());
    },
