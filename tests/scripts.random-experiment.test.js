...
describe('random-experiment.js', () => {
  ...
  test('should apply an experiment successfully if not already applied', () => {
    ...
    const lastExperimentCall = localFs.writeFileSync.mock.calls.find(
      (call) => call[0] === 'last-experiment.json'
    )
    ...
    expect(mockLog).toHaveBeenCalledWith(
      expect.stringContaining('Experiment applied successfully!')
    )
  })
  ...
  test('should skip if experiment is already applied', () => {
    ...
    expect(mockLog).toHaveBeenCalledWith(
      expect.stringContaining('Experiment already exists or not applicable')
    )
  })
  ...
  describe('experiments application logic', () => {
    ...
    test.each(cases)(
      'experiment $idx: $name applies successfully',
      ({ idx, name, checkString }) => {
        const mod = require('../scripts/random-experiment')
