...
- **Static Analysis Evasion in Tests:** Security scanners may flag literal string definitions (e.g., `'__proto__'`) as potential prototype pollution vectors, even when they are harmlessly used as test payloads in unit tests. To avoid false positives without disabling the scanner entirely, obfuscate the literal string by constructing it dynamically at runtime (e.g., `['__pro', 'to__'].join('')`).
