...
describe('deploy.js', () => {
    ...
    describe('deployTo', () => {
        ...
        test('デプロイ成功時にresolveする', async () => {
            ...
            const validToken = process.env.TEST_TOKEN || 'a'.repeat(32);
            await expect(
        ...
        test('deployTo API catch error with status 500', async () => {
            ...
            const MOCK_API_KEY = process.env.TEST_TOKEN || 'a'.repeat(32);
            await expect(deployTo('TEST', '/api', MOCK_API_KEY, {})).rejects.toThrow('TEST deployment failed');
        ...
        test('deployTo API catch error with status 200 (json parse error)', async () => {
            ...
            const MOCK_API_KEY = process.env.TEST_TOKEN || 'a'.repeat(32);
            await expect(deployTo('TEST', '/api', MOCK_API_KEY, {})).resolves.toBeUndefined();
        });
    });
});
