...
function applyRegexReplace (file) {
  ...
  if (file === 'tests/utils.planning.test.js') {
    ...
    content = content.replace(
      /expect\(typeof utilsPlanning\.findOpenSpaces\)\.toBe\('function'\)/g,
      "expect(typeof utilsPlanning.findOpenSpaces).toBe('function');"
    )
    content = content.replace(
      /mockRoom\.find\.mockReturnValue\(\[\{ id: 'source1' \}\]\)/g,
      "mockRoom.find.mockReturnValue([{ id: 'source1' }]);"
    )
    content = content.replace(/expect\(pos\)\.toBeNull\(\)/g, 'expect(pos).toBeNull();')
    content = content.replace(
      /const tiles = utilsPlanning\.getTilesAtDistance\(mockRoom, centerPos, 3\)/g,
  ...
  if (file === 'utils.planning.js') {
    content = content.replace(
      /openSpaces\.push\(\{ x, y, size: minSize \}\)/g,
      'openSpaces.push({ x, y, size: minSize });'
    )
    content = content.replace(
      /const sourceDist = Math\.min\(\.\.\.sources\.map\(\(s\) => pos\.getRangeTo\(s\)\)\)/g,
    ...
    content = content.replace(
      /console\.log\(`···Open spaces \(5x5\+\): \$\{openSpaces\.length\}`\)/g,
      'console.log(`   Open spaces (5x5+): ${openSpaces.length}`);'
    )
    content = content.replace(
      /console\.log\(`\\n🏗️ Room Planning \[\$\{room\.name\}\]:`\)/g,
      'console.log(`\\n🏗️ Room Planning [${room.name}]:`);'
    )
    content = content.replace(
      /console\.log\(` {3}Best spawn position: \$\{bestSpawnPos\}`\)/g,
      'console.log(`   Best spawn position: ${bestSpawnPos}`);'
    )
    content = content.replace(
      /return \{ openSpaces, bestSpawnPos \}/g,
  ...
  if (file === 'tests/spawnManager.test.js') {
    ...
    content = content.replace(
      /isSafeKey: jest\.fn\(\)\.mockReturnValue\(true\)/g,
...
['tests/utils.planning.test.js', 'utils.planning.js', 'tests/spawnManager.test.js'].forEach(
  applyRegexReplace
)
